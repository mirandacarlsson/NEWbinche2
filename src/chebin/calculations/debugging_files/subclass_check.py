...
def count_connected_classes(class_iri):
    ...
    with open(chebi_subclass_json) as f:
        subclass_map = json.load(f)

    with open(chebi_leafdescendant_json) as f:
    ...
    print(
        f"Class {class_iri} has {len(subclasses)} subclasses and {len(leaf_descendants)} leaf descendants.",
    )
    ...
    print(
        f"Out of these, {subclasses_in_removed} subclasses and {leaves_in_removed} leaf descendants are in the removed classes.",
    )
...
def check_leaves_of_class(class_iri):
    ...
    with open(leaf_descendants_json) as f:
        leaf_descendants_map = json.load(f)
    leaf_descendants = leaf_descendants_map.get(class_iri, [])
...
def count_removed_leaves():
    ...
    removed_classes = pd.read_csv(removed_leaves_csv)
    structure_count = len(
        removed_classes[removed_classes["Classification"] == "structural"],
    )
    function_count = len(
        removed_classes[removed_classes["Classification"] == "functional"],
    )
    print(f"Total removed leaf classes: {len(removed_classes)}")
    print(f" - Structural (Structure): {structure_count}")
    ...
    functional_classes = removed_classes[
        removed_classes["Classification"] == "functional"
    ]
...
def find_roles_for_leaf(leaf_iri):
    ...
    with open(leaf_roles_json) as f:
...
if __name__ == "__main__":
    ...
    with open(leaf_descendants_map_file) as f:
        leaf_descendants_map = json.load(f)
